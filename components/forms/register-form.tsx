...
import { RHFPasswordInput } from "./rhf-password-input"; // ✅ input com olhinho
import { authRegister } from "@/app/service/auth";
...
export const registerSchema = z
  .object({
    ...
    email: z.string().min(1, "Informe seu email").email("Informe um email válido."),
    password: z.string().min(4, "A senha precisa ter no mínimo 4 caracteres."),
    confirmPassword: z.string().min(4, "A senha precisa ter no mínimo 4 caracteres."),
  })
...
export function RegisterForm({
  ...
  async function onSubmit(values: RegisterFormData) {
    const id = notify.loading("Criando sua conta...");
    try {
      const user = await authRegister(values);
      if (!user.success) return notify.updateError(id, user.message);
      notify.updateSuccess(id, user.message);
      methods.reset();
      methods.clearErrors();
      router.push("/login");
    } catch {
      notify.updateError(id, "Erro ao criar usuário.");
    }
  }
